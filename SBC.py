...
def change_btn():
        ...
        if filedialog.Open():
            ...
            if q2 == 'yes':
                ...
                if number == '1':
                    ...
                    msgbox.showerror(
                        "관문 번호 로딩 오류", '관리자에게 "관문 번호 로딩 오류" 라고 전달해주세요')
                with open(exe_path+'/MainFolder/filepath.txt', 'w', encoding='UTF-8') as file:
                    file.write(file_list)
                msgbox.showinfo(number+'번 관문 BGM 설정 변경 완료', number +
                                '번 관문의 BGM 파일 설정이 '+file_name+'으로 변경되었습니다.')
        ...
        if q3 == 'yes':
            ...
            if number == '1':
                ...
                msgbox.showerror(
                    "관문 번호 로딩 오류", '관리자에게 "관문 번호 로딩 오류" 라고 전달해주세요')
                webbrowser.open('http://pf.kakao.com/_laxars/chat')
            msgbox.showinfo(number+'번 관문 BMG 파일 설정 초기화 완료',
                            number+'번 관문의 BGM 파일 설정이 애국가로 초기화 되었습니다.')
    ...
    Button(toplevel, bg='White', width=10, height=2, text='모두 초기화',
           command=all_change_btn).grid(row=1, column=2)
    Button(toplevel, bg='White', width=10, height=2, text='1번 바꾸기',
           command=lambda: change_btn('1')).grid(row=2, column=1)
    Button(toplevel, bg='White', width=10, height=2, text='2번 바꾸기',
           command=lambda: change_btn('2')).grid(row=3, column=1)
    Button(toplevel, bg='White', width=10, height=2, text='3번 바꾸기',
           command=lambda: change_btn('3')).grid(row=4, column=1)
    Button(toplevel, bg='White', width=10, height=2, text='4번 바꾸기',
           command=lambda: change_btn('4')).grid(row=5, column=1)
    Button(toplevel, bg='White', width=10, height=2, text='1번 초기화',
           command=lambda: reset_btn('1')).grid(row=2, column=3)
    Button(toplevel, bg='White', width=10, height=2, text='2번 초기화',
           command=lambda: reset_btn('2')).grid(row=3, column=3)
    Button(toplevel, bg='White', width=10, height=2, text='3번 초기화',
           command=lambda: reset_btn('3')).grid(row=4, column=3)
    Button(toplevel, bg='White', width=10, height=2, text='4번 초기화',
           command=lambda: reset_btn('4')).grid(row=5, column=3)
...
def up_btn():
    ...
    chk1 = IntVar()
    chk1_btn = Checkbutton(toplevel, text="항상 맨 위로",
                           variable=chk1, bg='white', command=up)
...
def transparency_btn():
    ...
    slide_bar = Scale(toplevel, from_=0.1, to=1.0, resolution=0.01, label='투명도', orient=HORIZONTAL,
                      showvalue=FALSE, relief='groove', sliderlength=20, sliderrelief='groove', command=slide)
...
def apply_btn_cmd():
    ...
    if 'exist' in exist:
        ...
        original = ['siroco_broken_d.ogg', 'siroco_broken_o1.ogg',
                    'siroco_broken_o2.ogg', 'siroco_broken_r.ogg']
        for file in file_list:
            num = file_list.index(file)
            if original[num] != file:
                shutil.copyfile(file, abs_path+'/'+original[num])
            elif original[num] == file:
                shutil.copyfile(exe_path+'/After/'+file,
                                abs_path+'/'+original[num])
            else:
...
btn1 = Button(root, bg='White', width=10, height=2, text="적용하기",
              relief='groove', command=apply_btn_cmd)
btn1.place(x=10, y=15)
btn2 = Button(root, bg='white', width=10, height=2, text="되돌리기",
              relief='groove', command=return_btn_cmd)
btn2.place(x=100, y=15)
root.mainloop()

# pyinstaller --clean -w --uac-admin --icon=.\DnF\SBC\SBC\MainFolder\icon.ico --add-data='.\DnF\SBC\SBC\After\*;After' --add-data='.\DnF\SBC\SBC\Before\*;Before' --add-data='.\DnF\SBC\SBC\MainFolder\*;MainFolder' .\DnF\SBC\SBC\SBC.py
